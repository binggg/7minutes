...
Game9G = function(gameid) {
	this.gameid = gameid;
	this.spid = null;
	this.baseurl = "http://www.faxianjie.com/";
	this.homeurl = "http://m.faxianjie.com/gzh/gzhstat?title=undefined&url=http%3A//mp.weixin.qq.com/s%3F__biz%3DMjM5MTY4Mzk0MA%3D%3D%26mid%3D200492249%26idx%3D1%26sn%3D5fa6de2b9e549bc1ba1d83e517e30f1f%23rd";
	this.gzurl = null;
	this.score = null;
	this.scoreName = null;
	this.shareDomain = null;
	this.shareDomains = ["www.faxianjie.com"];
	this.shareData = {
		imgurl: null,
		link: null,
		title: "发现街",
...
Game9G.prototype.init = function() {
    ...
    //首页url，如果有spid，就把spid写入
	this.homeurl = this.baseurl + "list.html" + (this.spid ? "?spid=" + this.spid : "");

	//公众url
    //this.gzurl = this.baseurl + "goto9g.html?f=" + this.gameid + (this.spid ? "&spid=" + this.spid : "");

    //分享的图片地址为游戏id下的icon.png
	this.shareData.imgurl = this.baseurl + this.gameid + "/icon.png";

    //分享的游戏链接为游戏id下的game.html
    this.shareData.link = this.baseurl + this.gameid + "/game.html?gameid=" + this.gameid + (this.spid ? "&spid=" + this.spid : "") + (localStorage.myuid ? "&uid=" + localStorage.myuid : "");
    ...
    //将分享数据写到window对象，ios端获取
    window.shareData = {
        "imgUrl": this.shareData.imgurl,
        "timeLineLink": this.shareData.link,
        "tTitle": this.shareData.title,
        "tContent": this.shareData.content
    };
...
Game9G.prototype.submit = function(callback) {
    if (this.cansubmit && this.score != null) {
        var _this = this;
        var name = localStorage.name || prompt("大侠，敢不敢告诉我你的名字","匿名用户");
        localStorage.name = name;
        setTimeout(function(){
            window.location = "http://m.faxianjie.com/game/gametop?gameid=" + _this.gameid + "&uid=" + name + "&score=" + _this.score + "&scorename=" + encodeURIComponent(_this.scoreName);
...
//		var _this = this;
//		setTimeout(function(){
//			if (confirm("要将成绩提交到发现街排行榜吗？")) {
//                //var uid = prompt("你的名字");
//				window.location = "http://wx.9g.com/rank/submit2.jsp?gameid=" + _this.gameid + (_this.spid ? "&spid=" + _this.spid : "") + "&uid=" + localStorage.myuid + "&score=" + _this.score + "&scorename=" + encodeURIComponent(_this.scoreName);
//			}
//			else {
...
Game9GUtils.prototype.showShare = function() {
    ...
    //不存在，就创建
		...
		img.id = "game9gshare";
		img.src = "http://www.faxianjie.com/share.png";
